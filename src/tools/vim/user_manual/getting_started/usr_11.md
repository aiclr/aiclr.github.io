...
```
Compare the file with the original file to check if you ended up with what you expected. Vimdiff is very useful for this [08.7](usr_08.md#087-viewing-differences-with-vimdiff). For example:
```text
        :write help.txt.recovered
        :edit #
        :diffsp help.txt
...
If this still does not work, see what file names Vim reports and rename the files accordingly. Check the [directory](../../options.md#directory-dir) option to see where Vim may have put the swap file.

Note:<br>
Vim tries to find the swap file by searching the directories in the [dir](../../options.md#directory-dir) option, looking for files that match `filename.sw?`. If wildcard expansion doesn't work (e.g., when the [shell](../../options.md#shell-sh-e91) option is invalid), Vim does a desperate try to find the file `filename.swp`. If that fails too, you will have to give the name of the swapfile itself to be able to recover the file.
...
If you really don't want to see this message, you can add the `A` flag to the [shortmess](../../options.md#shortmess-shm) option. But it's very unusual that you need this.

For remarks about encryption and the swap file, see [:recover-crypt](../../recover.md#recover-crypt).<br>
For programmatic access to the swap file, see [swapinfo()](../../builtin.md#swapinfo).
...
- [:swapname](../../recover.md#sw-swapname)  See the name of the swap file for the current file.
- [updatecount](../../options.md#updatecount-uc)   Number of key strokes after which the swap file is flushed to disk.
- [updatetime](../../options.md#updatetime-ut)   Timeout after which the swap file is flushed to disk.
- [swapsync](../../options.md#swapsync-sws)   Whether the disk is synced when the swap file is flushed.
- [directory](../../options.md#directory-dir)   List of directory names where to store the swap file.
- [maxmem](../../options.md#maxmem-mm)   Limit for memory usage before writing text to the swap file.
- [maxmemtot](../../options.md#maxmemtot-mmt)   Same, but for all files in total.
