...
```text
        ...
        +------------------+             +------------------+
```
The `zt`command put the cursor line at the top, `zb` at the bottom. There are a few more scrolling commands, see [Q_sc](../../quickref.md#q_sc-scrolling). To always keep a few lines of context around the cursor, use the `scrolloff` option.
