...
If you always want to use syntax highlighting, put the `:syntax enable` command in your [vimrc](../../starting.md#vimrc-exrc) file.

If you always want syntax highlighting only when the terminal supports colors, you can put this in your [vimrc](../../starting.md#vimrc-exrc) file:
```text
        if &t_Co > 1
           syntax enable
        endif
```
If you want syntax highlighting only in the GUI version, put the `:syntax enable` command in your [gvimrc](../../gui.md#gvimrc) file.
...
Vim will use bold, italic and underlined text, but this doesn't look very nice. You probably will want to try to get a terminal with colors. For Unix, I recommend the xterm from the XFree86 project: [xfree-xterm](../../syntax.md#xfree-xterm).
...
The terminal name must match the terminal you are using. If it still doesn't work, have a look at [xterm-color](../../syntax.md#xterm-color-color-xterm), which shows a few ways to make Vim display colors (not only for an xterm).
...
For a real fix, see [:syn-sync](../../syntax.md#syn-sync-e403-e404). Some syntax files have a way to make it look further back, see the help for the specific syntax file. For example, [tex.vim](../../syntax.md#tex) for the Tex syntax.
...
When you found the color scheme that you like, add the `:colorscheme` command to your [vimrc](../../starting.md#vimrc-exrc) file.
...
```
Attributes you can use for **cterm** and **gui** are **bold** and **underline**. If you want both, use **bold,underline**. For details see the [:highlight](../../highlight-hi-e28-e411-e415.md) command.

- Tell Vim to always use your color scheme. Put this line in your [vimrc](../../starting.md#vimrc-exrc):
...
```
You will see text in various color combinations. You can check which ones are readable and look nice. These aren't the only colors available to you though. You can specify `#rrggbb` hex colors and you can define new names for hex colors in [v:colornames](../../eval.md#vcolornames) like so:
```text
...
```
This allows users of the color scheme to override the precise definition of that color prior to loading your color scheme. For example, in a [.vimrc](../../starting.md#viminit) file:
```text
        runtime colors/lists/css_colors.vim
        let v:colornames['your_red'] = v:colornames['css_red']
        colorscheme yourscheme
```
As a color scheme author, you should be able to reply on some color names for GUI colors. These are defined in ***colors/lists/default.vim***. All such files found on the [runtimepath](../../options.md#runtimepath-rtp-vimfiles) are loaded each time the colorscheme command is run. A canonical list is provided by the vim distribution, which should include all X11 colors (previously defined in rgb.txt).
...
```text
        :syntax manual
```
This will enable the syntax highlighting, but not switch it on automatically when starting to edit a buffer. To switch highlighting on for the current buffer, set the [syntax](../../options.md#syntax-syn) option:
```text
        :set syntax=ON
```
...
- [printdevice](../../options.md#printdevice-pdev)
- [printheader](../../options.md#printheader-pheader)
- [printfont](../../options.md#printfont-pfn)
- [printoptions](../../options.md#printoptions-popt)
...
```text
       write main.c.html
```
Open this file in your favorite brower and print it from there. If all goes well, the output should look exactly as it does in Vim. See [2html.vim](../../syntax.md#convert-to-html) for details. Don't forget to delete the HTML file when you are done with it.
