...
function formatElapsedTime(ms) {
  ...
  const minutesText =
    minutes > 0
      ...
      : "";
  const secondsText = `${seconds.toString().padStart(2, "0")} second${seconds !== 1 ? "s" : ""}`;

  // 组合文本（处理空值和连接符）
  const parts = [daysText, hoursText, minutesText, secondsText].filter(Boolean);
  const result = parts.join(" · ");
  return result;
}

const MEET_TIME = new Date("2021-12-18T20:53:00");
const BIRTH_TIME = new Date("2024-06-25T13:15:00");

/**
 * 获取根据当前秒数切换基准时间的流逝时长文本
 * @returns {string} 最终显示的时长文本
 */
function getElapsedTimeText() {
  const now = new Date();
  // 选择基准时间：当前秒数<30用birthTime，否则用meetTime
  const targetTime = now.getSeconds() < 30 ? BIRTH_TIME : MEET_TIME;
  // 计算时间差（确保为正数）
  const elapsedMs = Math.max(0, Math.floor(now - targetTime));
...
function time() {
  const now = new Date();
  let ms = Math.floor(now - MEET_TIME);
  if (now.getSeconds() < 30) {
    ms = Math.floor(now - BIRTH_TIME);
  }
  ...
  if (h > 0) hoursText = h.toString() + " hours,";
  ...
  return (
    daysText + hoursText + minutesText + secondsText + " seconds have passed"
  );
}

/**
 * 安全更新标题内容
 * @param {string} content 要显示的新内容
 */
function updateMenuTitle(content) {
  // 获取目标元素
  const titleElement = document.querySelector(".menu-title");

  // 空值检查：元素不存在时提示并终止
  if (!titleElement) {
    console.warn("未找到class为menu-title的h1元素");
    return;
  }
...
function init() {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // 初始化显示第一个标题
    updateMenuTitle("希君生羽翼 一化北冥鱼");

    // 启动定时刷新任务
    setInterval(() => {
      const nextTitle = getElapsedTimeText();
      updateMenuTitle(nextTitle);
    }, 3000);
  });
}
