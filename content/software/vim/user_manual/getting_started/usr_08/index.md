...
Next chapter: [usr_09.txt](../usr_09)  Using the GUI<br>
Previous chapter: [usr_07.txt](../usr_07)  Editing more than one file<br>
...
```text
        ...
        +----------------------------------+
```
What you see here is two windows on the same file. The line with `====` is the status line. It displays information about the window above it. (In practice the status line will be in reverse video.)<br>
...
The [winheight](../../../options#winheight-wh-E591) option can be set a minimal desired height of a window and [winminheight](../../../options#winminheight-wmh) to a hard minimum height.<br>
Likewise, there is [winwidth](../../../options#winwidth-wiw-E592) for the minimal desired width and [winminwidth](../../../options#winminwidth-wmw) for the hard minimum width.<br>
The [equalalways](../../../options#equalalways-ea-noequalalways-noea) option, when set, makes Vim equalize the windows sizes when a window is closed or opened.
...
```
You will notice the same letters as used for moving the cursor. And the cursor keys can also be used, if you like.<br>
More commands to move to other windows: [Q_wi](../../../quickref#q_wi-multi-window-commands).
...
```text
        CTRL-W K
```
This uses the uppercase letter K. What happens is that the window is moved to the very top. You will notices that *K* is again used for moving upwards.<br>
...
```text
        :wall
```
This stands for `write all`. But actually, it only writes files with changes. Vim knows it doesn't make sense to write files that were not changed.<br>
And then there is the combination of `:qall` and `:wall`: the *write and quit all* command:
```text
        :wqall
```
This writes all modified files and quits Vim.<br>
...
```text
        ...
        +-------------------------------+
```
The `-O` argument is used to get vertically split windows.<br>
...
The lines that were not modified have been collapsed into one line. This is called a closed fold. They are indicated in the picture with *<- fold*. Thust he single fold line at the top stands for 123 text lines. These lines are equal in both files.<br>
...
```text
        dp
```
The change will be removed by putting the text of the current window in the other window. `dp` stands for `diff put`.<br>
...
For details about diff mode, see [vimdiff](../../../diff)
...
The [laststatus](../../../options#laststatus-ls) option can be used to specify when the last window has a statusline:
```text
        0       never
        1       only when there are split windows (the default)
        2       always
```
Many commands that edit another file have a variant that splits the window. For Command-line commands this is done by prepending an *s*. For example: `:tag` jumps to a tag, `:stag` splits the window and jumps to a tag.<br>
...
The [splitbelow](../../../options#splitbelow-sb-nosplitbelow-nosb) option can be set to make a new window appear below the current window. The [splitright](../../../options#splitright-spr-nosplitright-nospr) option can be set to make a vertically split window appear right of the current window.
